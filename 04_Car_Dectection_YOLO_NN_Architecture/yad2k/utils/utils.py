...
def preprocess_image(img_path, model_image_size):
    # PIL itself can tell us the format (JPEG, PNG, etc.) if you ever need it:
    # image_type = Image.open(img_path).format
...
def draw_boxes(image, boxes, box_classes, class_names, scores=None):
    ...
    # Normalize scores to a plain numpy array (or None) once, up front.
    ...
    for i, c in list(enumerate(box_classes)):
        ...
        for t in range(thickness):
            draw.rectangle(
                [left + t, top + t, right - t, bottom - t], outline=colors[c])
        draw.rectangle(
